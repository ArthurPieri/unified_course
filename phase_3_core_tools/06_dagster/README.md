...
| `Resource 'dbt' required by asset not found` | Asset uses `dbt: DbtCliResource` but `Definitions(resources=...)` is missing the key | Add `resources={"dbt": DbtCliResource(project_dir=...)}` to the `Definitions` call | [Dagster — dagster-dbt reference](https://docs.dagster.io/integrations/dbt/reference) |
