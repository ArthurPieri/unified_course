...
Compose declares a set of services in one YAML file. Every `image:` tag should be pinned to a specific version — the `../dataeng/docker-compose.yml` reference uses `postgres:16-alpine`, `trinodb/trino:470`, `apache/hive:4.0.1`, `metabase/metabase:v0.51.0` (see `../dataeng/docker-compose.yml:L9`, `:L63`, `:L92`, `:L187`). `postgres:latest` is a moving target; a silent major-version bump can break schemas. Secrets come from a `.env` file sitting next to `docker-compose.yml`; Compose reads it automatically. Reference secrets with `${VAR}` or `${VAR:-default}` in the YAML so the file itself stays committable. The `.env` file must be in `.gitignore` — commit a `.env.example` with placeholder values instead.
Ref: [Compose file — services](https://docs.docker.com/reference/compose-file/services/) · [Compose — environment variables](https://docs.docker.com/compose/how-tos/environment-variables/) · `../dataeng/docker-compose.yml:L10-L14`

### Healthchecks and `depends_on: condition: service_healthy`
A healthcheck is a command Compose runs inside the container at a fixed `interval`; a non-zero exit for `retries` in a row marks the container `unhealthy`. `start_period` gives slow boots a grace window where failing probes do not count against `retries`. Other services that declare `depends_on: <name>: condition: service_healthy` are held back until the dependency reports healthy — this is how the dataeng stack guarantees `hive-metastore` does not start before `metastore-db` is accepting connections (`../dataeng/docker-compose.yml:L67-L72`, `:L83-L88`). Canonical probes: `pg_isready` for Postgres (`../dataeng/docker-compose.yml:L19-L23`), an HTTP request for MinIO (`../dataeng/docker-compose.yml:L39-L43`), a TCP check with `nc -z` for thrift-style services (`../dataeng/docker-compose.yml:L83-L88`).
...
### Profiles
`profiles:` marks a service as optional — it only starts when you run `docker compose --profile <name> up`. The dataeng stack uses profiles to slice the full lakehouse into storage/metadata/query/orchestration/visualization subsets (`../dataeng/docker-compose.yml:L28`, `:L48`, `:L93`, `:L117`, `:L172`). A service with no `profiles` key runs unconditionally — the baseline `postgres` at `../dataeng/docker-compose.yml:L8-L23` has no profile, so every invocation starts it.
Ref: [Compose — profiles](https://docs.docker.com/compose/how-tos/profiles/)

### Resource limits
`mem_limit` caps a container's RAM; the kernel's OOM killer terminates processes that exceed it. The dataeng stack caps Postgres at 512m, a Hive metastore at 1g, and Trino at 5g (`../dataeng/docker-compose.yml:L18`, `:L82`, `:L106`). Without limits a runaway Spark driver can freeze a 16GB laptop. `cpus:` sets a fractional CPU share. Both keys belong at the service level.
...
| Container `unhealthy` but app works | Healthcheck command not installed in the image | Use `CMD-SHELL` with a tool known to exist (`pg_isready`, `wget`, `curl`) | `../dataeng/docker-compose.yml:L19-L23` |
