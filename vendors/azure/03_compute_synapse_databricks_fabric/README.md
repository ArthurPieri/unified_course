...
Ref: [Microsoft Learn: Delta Lake](https://learn.microsoft.com/en-us/azure/databricks/delta/) · [Delta MERGE](https://learn.microsoft.com/en-us/azure/databricks/sql/language-manual/delta-merge-into)
...
Ref: [Fabric Warehouse T-SQL surface area](https://learn.microsoft.com/en-us/fabric/data-warehouse/tsql-surface-area)
...
Ref: [KQL reference](https://learn.microsoft.com/en-us/kusto/query/)
...
Ref: [Dedicated SQL pool distributions](https://learn.microsoft.com/en-us/azure/synapse-analytics/sql-data-warehouse/sql-data-warehouse-tables-distribute) · [COPY INTO (Fabric Warehouse)](https://learn.microsoft.com/en-us/fabric/data-warehouse/ingest-data-copy)
...
Ref: [Azure Databricks](https://learn.microsoft.com/en-us/azure/databricks/)
...
|---|---|---|---|
| L03.1 Spark transformations | Filter, groupBy, join, broadcast, `.explain()` plan reading | 60 m | [Azure Databricks Spark](https://learn.microsoft.com/en-us/azure/databricks/getting-started/spark/) |
| L03.2 Delta MERGE | Upsert scenario with three-clause `MERGE`, time travel, history | 45 m | [Microsoft Learn: Delta Lake](https://learn.microsoft.com/en-us/azure/databricks/delta/) |
| L03.3 T-SQL on Warehouse | `COPY INTO`, `CTAS`-style pattern, JSON shredding, RLS | 90 m | [Fabric Warehouse T-SQL surface area](https://learn.microsoft.com/en-us/fabric/data-warehouse/tsql-surface-area) |
| L03.4 KQL exercises | Basic `where` / `summarize` / `render` against a KQL DB or Log Analytics demo | 60 m | [KQL tutorial](https://learn.microsoft.com/en-us/kusto/query/tutorial) |

## Common failures

| Symptom | Cause | Fix | Source |
|---|---|---|---|
| `OutOfMemoryError` on driver | `.collect()` / `.toPandas()` on a big DataFrame | Process in partitions, write to storage, or aggregate first | [Azure Databricks Spark](https://learn.microsoft.com/en-us/azure/databricks/getting-started/spark/) |
| One Spark task takes 100x longer than others | Data skew on the join key | Enable AQE skew join, salt the key, or broadcast the small side | [Adaptive Query Execution](https://learn.microsoft.com/en-us/azure/databricks/optimizations/aqe) |
| `MERGE` fails with "ambiguous rows" | Multiple source rows match the same target row | Dedupe source first (ROW_NUMBER + filter) | [Microsoft Learn: Delta Lake](https://learn.microsoft.com/en-us/azure/databricks/delta/) |
